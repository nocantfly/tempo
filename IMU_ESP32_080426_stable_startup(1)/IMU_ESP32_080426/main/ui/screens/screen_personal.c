...
static void style_card_base(lv_obj_t *obj)
{
    ...
    // pad nho de 4 cell an gan full card
...
lv_obj_t *screen_personal_create(void)
{
    ...
    lv_label_set_text(label_title, "Thiet bi dinh huong IMU va theo doi");
...
void screen_personal_set_card_color(lv_color_t main, lv_color_t bg)
{
...
void screen_personal_update_imu(float roll, float pitch, float yaw)
{
    ...
    if (err < 5.0f)
    {
        screen_personal_set_status("Chuan hoa bo cam bien: hoan tat");
        screen_personal_set_card_color(
            lv_color_hex(0x39FF5A),
            lv_color_hex(0x1A2318));
    }
    else if (err < 15.0f)
    {
        screen_personal_set_status("Chuan hoa bo cam bien: can chinh nhe");
        screen_personal_set_card_color(
    ...
    {
        screen_personal_set_status("Chuan hoa bo cam bien: lech nhieu");
        screen_personal_set_card_color(
